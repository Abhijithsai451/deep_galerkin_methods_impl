...
def plot_2d_solution_surface(solver_instance, domain_bounds, analytical_solution_func=None, time_slice=0.0,
    ...
    if analytical_solution_func:
        ...
        x_flat = torch.from_numpy(x_flat_np).float().to(solver_instance.device)
        t_flat = torch.from_numpy(t_flat_np).float().to(solver_instance.device)

        u_analytical_flat = analytical_solution_func(x_flat, t_flat, plot_params.get('analytical_params', {}))
        U_analytical = u_analytical_flat.cpu().numpy().reshape(num_points, num_points)
