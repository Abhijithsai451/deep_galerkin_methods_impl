...
def plot_1d_(solver, alpha, domain_bound,time_bound):
    ...
    t_test = np.linspace(time_bound[1][0],time_bound[1][1],5)[:, None]
...
def plot_2d_(solver, domain_bound,time_point):
    ...
    Spatial_coords_flat_tensor = torch.tensor(Spatial_coords_flat_np,dtype=torch.float32)
    t_flat_tensor = torch.tensor(t_flat_np,dtype=torch.float32)
...
def plot_3d_(solver, alpha, domain_bound,time_point,fixed_coord_dim, fixed_coord_val):
    """
    ...
    spatial_coord_flat_tensor = torch.tensor(spatial_coord_flat_np,dtype=torch.float32)
    t_flat_tensor = torch.tensor(t_flat_np,dtype=torch.float32)
    ...
    U_pred = predict.reshape(n_points_slice, n_points_slice)
    ...
    ax.set_zlabel('u')
    ax.set_title(f'3d Solutionat t={time_point:.2f}\n Fixed{['x','y','z'][fixed_coord_dim]}={fixed_coord_val:.2f}')
    ax.view_init(elev=20, azim=60)
    plt.tight_layout()
    plt.show()


def analytic_func(x_tensor, t_tensor, alpha):
    return None