...
def f_func_1d(x_coords):
    return -4 * torch.pi ** 2 * torch.sin(2 * torch.pi * x_coords[:, 0:1])


def bc_1d_poisson(spatial_coords, lx_1d):
    x = spatial_coords[:, 0:1]
    tolerance = 1e-6
    result = torch.zeros_like(x)

    # Boundary conditions --> u(0) = 0, u(lx_1d) = 0
    # result[torch.abs(x - lx_1d) < tolerance] = 0.0
    return result
...
# <<<<<<--------- All Utility Functions for 2D poissons equations
def f_func_2d(coords):
    # For analytical solution u(x,y) = u(x,y)=sin(2πx)sin(2πy)
    # Laplacian(u) = d^2u/dx^2 + d^2u/dy^2 = −8pi**2* sin(2πx)sin(2πy)
    x, y = coords[:, 0:1], coords[:, 1:2]
    laplacian = -8 * torch.pi ** 2 * torch.sin(torch.pi * x) * torch.sin(torch.pi * y)
    return laplacian
...
def analytical_solution_2d_poisson(x: torch.Tensor, y: torch.Tensor, pde_params: dict = None):
